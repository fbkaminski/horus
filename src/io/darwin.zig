...
const buffer_limit = @import("common.zig").buffer_limit;
const DirectIO = @import("common.zig").DirectIO;
        ...
        self.submit(
            ...
            struct {
                fn do_operation(op: anytype) RecvError!usize {
                    return posix.recv(op.socket, op.buffer[0..op.len], 0);
                }
            },
        );
    }
        ...
        self.submit(
            ...
            struct {
                fn do_operation(op: anytype) WriteError!usize {
                    ...
                    // completed.
                    const result = posix.pwrite(op.fd, op.buffer[0..op.len], op.offset);
                    try fs_sync(op.fd);
